...
I captured screenshots of the running app for submission evidence. They are saved under `tests/e2e/screenshots/`:

- `register_page.png` — registration page UI
- `login_page.png` — login page UI
- `after_register_<ts>.png` — confirmation after a successful registration
- `after_login_<ts>.png` — post-login UI showing success message (token stored in localStorage)
- `docs.png` — OpenAPI docs (evidence of protected endpoints and Authorize button)

## If you want these embedded into the README for a GitHub-hosted repo, I can add them inline (or upload them to a visible location). For now they are included in the repository under `tests/e2e/screenshots/`.
